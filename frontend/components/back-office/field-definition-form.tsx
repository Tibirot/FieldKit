 ...
 *
 * The key is an identifier — it goes into JSON and into future index expressions — while the label
 * is prose, so left to themselves an admin types "Chiller count" into both and the server refuses
 * the second. Deriving it removes the mismatch at the source rather than explaining it afterwards.
 *
 ...
 *
 * Exported for its own tests: it runs on every keystroke of a field whose output is immutable
 * afterwards, which is a poor place to discover an edge case.
 */
