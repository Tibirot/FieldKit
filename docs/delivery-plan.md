...
| | *Shipped. **Two choices worth arguing with**, both pinned by sabotage. A push the server refuses **whole** is still measured — the workload a reconnect carries is what the device *tried* to send, and a histogram that drops its own outliers looks healthiest exactly when devices are misbehaving. And a rejection is counted where **only a first attempt reaches**, not where the refusal is decided: counting the replay would make the rejection rate a measurement of a device's retry policy, climbing on its own with nothing happening in the field and climbing fastest for the rep whose connection is worst. Latency comes from a monotonic `Stopwatch` timestamp rather than `IClock` — a wall clock can step, and an NTP correction mid-push would produce a negative duration.* | | |
| | ***The first sabotage passed, which is the finding.** Replacing the handler's `try`/`finally` with sequential statements changed nothing: every refusal in `/sync/push` **returns** rather than throws, so the only thing `finally` adds is coverage of a thrown push — a database that went away mid-batch — and no test provokes one. The test that looked like it pinned the `finally` actually pins the recording living **outside** the guarded body; moving it past the guards is what fails. The block stays, because a latency number is most worth having when the request died, and the comment now says plainly that it is a defensible belief with nothing holding it up. Sabotaging what you assume a test covers is how you find out it covers something else.* | | |
...
| | *Shipped, and **the claim was wrong as well as unbuilt**. §4 promised a `traceId` "in every `ProblemDetails`" and **this API does not answer with `ProblemDetails`** — every refusal it raises uses its own `{ "errors": [...] }` envelope (`api-contracts §3`), which had no trace id in it. So the id went where the refusals actually are, and the doc now names that shape; the `ProblemDetails` path a parse failure still takes carries the same spelling, set explicitly rather than left to the framework's default of `Activity.Id`, which is the **`traceparent`** and names one span rather than the trace. **The tenant is stamped on the request's own span**, not a child of it, so "everything this tenant did" is a filter rather than a join and costs no extra span — after authentication, because the tenant comes from a validated token and from nowhere else. **`mutationId` gets a span apiece**, which is where slice 1's rule sends it: the counter says how many were rejected, the span says which one. **Outbox dispatch is not in this slice** — there is no dispatcher to open a span (slice 3), recorded rather than quietly dropped.* | | |
| | ***A diagnostic run for a mistake worth writing down.*** *The test that finds the request span matched on `Activity.OperationName`, which for every ASP.NET request is the constant `Microsoft.AspNetCore.Hosting.HttpRequestIn` — the route lives in `DisplayName`. It failed as "collection was empty", which reads like "the tenant was never stamped" and was nothing of the kind. Also caught by the same run: `PullRequest(Guid, PullCursors?)` has **no default on `Cursors`**, so an anonymous `{ deviceId }` body fails to bind and the endpoint never runs — the exact trap `PushedMutation`'s own remarks warn about, met from the test side this time.* | | |
| | ***A second sabotage that passed, and a test asserting the wrong path.*** *Removing the `IsAuthenticated` guard from the tenant stamp changed nothing — because the test used an unauthenticated `/sync/push`, and `UseAuthorization` short-circuits a 401 **before** this middleware runs. The guard only matters where a request is let through **without** a principal, so the test now uses `/alive`; with the guard gone it answers **500**, which is the incident the guard exists to prevent (`ITenantContext` throws rather than inventing a tenant). Two slices running, two sabotages that passed — both times because the test was pinned to a path the code under test never took. That is now the thing to check first when writing one.* | | |
...
| | *Shipped. **Registered by `AddModuleDbContext`, not by a list in a host** — a list is a thing a module can be left off, and the module left off would go on committing events nobody delivers, which is the exact failure the slice ends. **Drain, then wait**: a full batch means more is behind it, so the loop goes straight round; a fixed interval would make the worst case `backlog ÷ batch × interval`, which for a burst is minutes of lag for nothing. The loop **catches and sleeps** rather than letting an exception escape, because an unhandled one from `ExecuteAsync` stops the service permanently and takes the host with it — a database blip at 3am would otherwise end delivery for the life of the process. **Three signals, and the third earned its place**: `dispatch.failed` is separate from the backlog because a queue high from a burst and one high from a broken subscriber look identical and need different answers.* | | |
| | ***A third sabotage that passed, and the test it fixed.** Reporting the batch's own `processed` count as the backlog left every test green — "reaches zero" converges either way once a queue drains. The two numbers only disagree where it matters: a **throwing handler**, where nothing is ever handled and the queue never empties. So the alert is now written as a test — a stuck message is reported as backlog `1`, and a dispatcher reporting its own throughput says `0` at the moment a module has stopped delivering entirely. Three slices, three sabotages that passed; each time the assertion converged on the same value by a different route, and the fix was to find the case where the routes part.* | | |
| | ***Two package references, both argued rather than assumed.*** *`Microsoft.Extensions.Diagnostics` in Infrastructure, so `AddModuleDbContext` can register the meter factory itself — "the host remembers" is the same shape of mistake as the dispatcher list. And `Microsoft.Extensions.Hosting` in `Infrastructure.Tests`, so the dispatcher test starts a **real host**: what was missing was never the processing, which had a test since W5, but anything starting it — so a test that called the loop directly would re-prove the working half and skip the broken one.* | | |
...
| | *Shipped, and **the tempting shape was the wrong one**. Visit and Order both raise integration events that — since slice 3 — something actually delivers, so handlers would have assembled these from facts the modules already publish, touching no domain code. Wrong twice: **business throughput must not depend on the outbox draining**, because a stalled dispatcher flattening "visits completed" reads as *reps have stopped working* rather than *delivery has stopped*; and neither event carries a tenant, so a handler could not tag by tenant without changing a published integration event for a metric's convenience. **The budget does not transfer either** — §6's sub-millisecond figure is the device's pure arithmetic, and the server side is three gathering queries whose cost grows with a tenant's price lists, so quoting it would produce an alert that fires on a healthy system.* | | |
| | ***The finding: a tenant-owned table cannot be sampled from a loop.*** *`photos.upload.pending` looked like the outbox backlog and is not. `PhotoEntry` is `ITenantOwned`, so the global query filter reads the ambient tenant — and a background service has no principal to read one from. Counting across tenants from a sampler would need `IgnoreQueryFilters` (banned at compile time) or a tenant list Audit has no business holding. **The isolation control that makes the app safe is the same thing that makes a tenant-wide aggregate impossible from outside a request**; the outbox escaped it only because `OutboxMessage` is not tenant-owned. The level is therefore recorded at the two events that move it — an audit arriving, a photograph confirmed — which is **exact** rather than sampled, because those are the only two things that change it.* | | |
...
| | *Shipped. **`self` was tagged `live` and was all either endpoint ran**, so readiness and liveness answered the same question and an instance that had lost its database reported ready. **None of the three new checks is tagged `live`** — a liveness probe that fails on a dependency asks the platform to restart a service that is working, and restarting every instance when the database blinks is how a blip becomes an outage. Postgres is checked **once** rather than eleven times (schema-per-module is eleven contexts over one database); Keycloak by fetching `master`'s discovery document, so a 200 means it is *serving OIDC* rather than merely accepting TCP; the outbox by heartbeat, because a loop that has died cannot report that it has died. **A module that has never beaten is not a failure** — a readiness check that fails during start-up keeps an instance out of rotation for a reason that will pass on its own, and on a platform that gives up after N probes that is a revision that never goes healthy.* | | |
| | ***The redaction the exposure decision seemed to need turned out to be the framework default.*** *Both endpoints now answer everywhere, because W15's platform probes one and would have met a 404. That looked like it required hiding the body — and **ASP.NET's default writer already emits one word**, so the Aspire guidance is about the endpoint *existing*, not about leaking. The work inverted: production needed nothing, and **development needed detail it did not have**, because "Unhealthy" with no further comment is a poor morning when three checks could each be the reason. The custom writer is therefore the **detailed** one.* | | |
| | ***And a test that documents rather than guards.*** *`A_module_that_has_never_reported_is_not_a_failure` asserts a property that follows from the data structure — no heartbeat entry means nothing to find stale — so no sabotage of the current code can fail it. It is kept because the behaviour is a **choice about the future**: it would catch a change that seeded the heartbeat with every module at start-up and then judged them, which is the obvious next edit and the one that breaks a deploy. Recorded as documentation rather than counted as coverage.* | | |
...
| | *Shipped, and **half the row's title names a path this server does not own**. There are no auth endpoints to limit: authentication is Keycloak's, which the browser talks to directly, and this API only validates JWTs — its one `/api/auth/*` route is an authenticated read of your own claims. Brute-force protection for sign-in is realm configuration. Same species as the CORS finding, found the same way, and now stated in `16-security.md` rather than implied. **`/sync` is limited per rep**, keyed on the subject in the token, so 200 reps are 200 budgets that never interact. The refusal arrives in this API's own `{ "errors": [...] }` envelope with an ADR-0012 code and a `Retry-After`, because the limiter's default is a 429 with an empty body and a device already has one branch for the envelope.* | | |
| | ***`/health` was limited and then deliberately un-limited.*** *Readiness does a database round trip and an OIDC fetch per request, and slice 5 made it reachable everywhere — a small amplifier pointed at two dependencies. But **a 429 is an unhealthy answer to a probe**: the limiter would have been able to take a working instance out of rotation, fastest under exactly the load it was added for, and the orchestrator probes from inside the network where it shares an address bucket with everything co-located. The control for that exposure is ingress. Built, then removed, and recorded rather than quietly dropped.* | | |
| | ***The number was wrong, and the test suite is what proved it.*** *Sixty a minute looked generous against a steady-state reconnect — a pull, a push, a photo confirmation. A **first** sync is not that: binding pages ten entity feeds of up to 500 rows, which a rep with a new phone does from cold, and the suite doing the same thing tripped the limit and failed six `SyncPullTests` with 429. Raised to **300** (five a second, sustained, per device). A limit that sits inside the range of ordinary behaviour is the one thing a limit must never be — and this one was, until a legitimate client walked into it.* | | |
| | ***And a test that broke its neighbours.*** *The first version exhausted the rep's budget in the collection's shared host; a fixed window is a minute wide, so whatever ran next was refused. A limiter's budget is **state, shared and slow to recover** — the fixture now lifts the limit for the shared host (a suite presenting three tokens is not three reps) and `RateLimitTests` stands up its own host with a limit of two. `ServerFixture.WithSettings` reuses the containers and nothing else.* | | |
...
| | *Shipped. **The sentence was wrong, not the code** — a browser never reaches this API cross-origin, because it calls `/api/*` on the **front end's own origin** and `proxy.ts` rewrites to the upstream. Adding a policy would mean naming origins that are *permitted*, which is strictly more permission than none. The test pins the **absence**: no `Access-Control-Allow-Origin` against a request carrying an `Origin`, and no answer to a preflight — so the day somebody adds CORS without meaning to, something fails.* | | |
| | ***And the API had no security headers at all***, *which no document claimed and nothing had noticed. `proxy.ts` sets them on documents and returns early for `/api/` — correct, since everything it does after that is about rendering — so the JSON side carried none. It has four now. `nosniff` is the one that matters for JSON: it makes a declared content type binding rather than a suggestion. `default-src 'none'` is for the response this API is not supposed to have, where something renders as HTML. And **`no-store` on every response**, because what a caller may read here depends on their token: a shared cache keyed on a URL is a cross-tenant read waiting for two people to ask the same question. **Set before the pipeline runs**, so a 401 that never reaches an endpoint carries them too — which is the response most likely to be produced by something other than this application's own code.* | | |
...
| | *Shipped, and **it goes to the telemetry pipeline rather than to a table** — a structured log per event plus a counter by kind, correlated by the tenant and trace the rest of W13 established. No schema, no migration, no retention policy to invent: this is diagnostics, not a record of work, and a business table would make it somebody's to keep and to erase. The **kinds are a closed vocabulary** and one this server does not know is refused rather than bucketed, so a client shipping a typo reports nothing loudly instead of something unreadable quietly.* | | |
| | ***No location, and the enforcement is structural rather than a rule.*** *`DeviceEvent` has three fields, so a client that sends latitude and longitude has them dropped by the deserializer before any code reads them — stronger than validation, because there is no branch to get wrong. The batch is still **accepted**: the device is not punished for a field this server does not want. What stays a client-side promise is the *content* of `detail`, and that is said rather than pretended away.* | | |
| | ***Two client-side choices worth arguing with.*** *The buffer **keeps the oldest and drops the newest** — the opposite of a ring buffer, because the first failure explains the rest: a store that was evicted and then threw a hundred quota errors should report the eviction. And it **clears only once the server has it**, because the interval where a device is failing is the interval where it cannot tell anybody. It lives in a `meta` row rather than an object store: a store means a Dexie version bump and a migration path (`OFF-13`) for data deleted the moment it is delivered.* | | |
