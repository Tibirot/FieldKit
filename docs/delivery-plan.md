...
| | **Not verified in the browser** *— Docker Desktop's daemon was down, so Postgres and Keycloak could not start. The evidence is 8 component tests with three sabotages (skipped pillar as 0%, pending steps dropped, weighting version hidden), typecheck, lint and the reachability gate. Worth a look on the running app before W12 closes.* | | |
