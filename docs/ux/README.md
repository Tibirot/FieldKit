...
| Visit-workflow / audit builder | `A1` `ADR-0009` `VIS-03` `AUD-04` `AUD-06` `AUD-07` | The config-driven story: per-channel steps, perfect-store weights, survey forms. **Weights** (W10 slice 8) and **survey forms** (W10 slices 9a/9b) are built as their own screens under a **Configuration** nav item the drawing does not have — see the note below. The survey editor **reorders with buttons rather than the drawn drag handle**, for the reason in [Configuration §6.3](../product/14-configuration.md#63-authoring-survey-forms-week-10). **The per-channel step half is the one part of this drawing with no screen behind it** (`CFG-03`) — its API has existed since W7 slice 6 and it is scheduled for [W14](../delivery-plan.md#week-14--e2e--seed-data--the-workflow-builder-), so this row is a wireframe two-thirds realised rather than a built screen ([Configuration §6.5](../product/14-configuration.md#65-authoring-visit-workflows-not-yet-built)) |
